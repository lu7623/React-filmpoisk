import { FullMovieInfo, IQueryParams, IResponse, ShortMovieInfo } from '@/api/types';
...
export const filmpoiskAPI = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getFilmsSearch: builder.query<IResponse, IQueryParams>({
      query: ({ title, release_year, genre, page=1, limit=10, order ='desc', sort_by = 'rating'  }) => ({
        url: '/search',
        params: {
          title: title,
...
export const { useGetFilmsListQuery, useGetFilmByIdQuery, useGetFilmsSearchQuery } = filmpoiskAPI; 
